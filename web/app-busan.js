...
const CANDIDATE_META = {
  "하정우":  { party: "더불어민주당", color: "#2f6fdb", short: "민주",   photo: "images/busan/하정우.webp" },
  "한동훈":  { party: "무소속",       color: "#b5bdcc", short: "무소속", photo: "images/busan/한동훈.webp" },
  "박민식":  { party: "국민의힘",     color: "#e74c5e", short: "국힘",   photo: "images/busan/박민식.webp" },
};
...
function renderGoldenCrossCard(trend) {
  ...
  if (gc.actual_observed_date) {
    statusEl.innerHTML = `🚨 <span style="color:${cColor}">${challenger}</span> 추세선이 <span style="color:${lColor}">${leader}</span>를 추월 — 골든크로스 발생 (${gc.actual_observed_date})`;
  } else {
  ...
  detailEl.innerHTML = `
    ...
    <div class="gc-line gc-dim">선형 외삽은 단순 추정치이며, 실제 막판 보수단일화·돌발 이슈에 따라 더 빨라지거나 안 일어날 수 있어요.</div>
  `;
}
...
async function init() {
  ...
  try {
    const [pred, timeline, trend] = await Promise.all([
      loadJSON("data/busan/predictions.json"),
      loadJSON("data/busan/history_timeline.json").catch(() => []),
      loadJSON("data/busan/poll_trend.json").catch(() => null),
    ]);
